...
- (void)setLoading:(BOOL)loading
{
    ...
    if (_loading) {
        ...
        [UIView transitionWithView:_scrollView
                          duration:0.3f//update by BruceZCQ
...
- (void)endRefresh
{
    ...
    if ([self.delegate respondsToSelector:@selector(slimeRefreshEndRefresh:)]) {
        [self.delegate slimeRefreshEndRefresh:self];
    }
